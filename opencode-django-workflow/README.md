# Django Development with Opencode

Personal opencode setup for structured Python/Django development, using
...
```bash
git clone <this-repo>
cd opencode-django-project
ln -s ~/my-skills/.config/opencode/skills/python-style ~/.config/opencode/skills/python-style
...
Session 2:  opencode                        (project-manager is default)
            architect -> django-developer -> tester -> archive objectives
```
...
grilled-down specification, an implementation plan, working
Python/Django code, and a pytest suite, all following the
conventions in `.project/STYLE_GUIDE.md`.
...
- Python >= 3.11
- Django >= 5.0
- Python packages: `pytest`, `pytest-django`, `black`, `isort` (or `ruff`)
- Dependencies pinned in `pyproject.toml` or `requirements.txt`
...
1. **Clone the repository**
   ...
   ```bash
   git clone <this-repo> opencode-django-project
   cd opencode-django-project
   ```
...
2. **Symlink skills**
   ...
   ln -s ~/my-skills/.config/opencode/skills/python-style \
     ~/.config/opencode/skills/python-style
   ...
   Note: `django-developer` and `tester` no longer load the
   `python-style` skill — the rules are inlined in their prompts so
   they cost no extra round trip. The symlink is still worth keeping
   for ad-hoc Python work outside the pipeline.
...
├── .project/
│   ├── STYLE_GUIDE.md              Python/Django conventions (canonical)
...
└── .opencode/
    ...
    │   ├── django-developer.md     Implementation engineer subagent
...
write `.project/TASKS/<task>.md`. The output includes type-hinted
function signatures, pseudocode, a file layout, data flow, and an
execution checklist split into `[IMPL]` and `[TEST]` items.

#### Django Developer

The `django-developer` subagent executes only `[IMPL]` checklist
items. The style rules are inlined in its prompt, so it reads only
the task file. It runs smoke tests via `python -m py_compile` on
every modified module and `python manage.py check` once, and must
never touch `tests/`. When re-invoked to fix code bugs it reads only
the `## Code Bug Fixes` section, the named failing tests, and the
named application modules.

#### Tester

The `tester` subagent writes `[TEST]` items, executes them with
`pytest tests/test_<module>.py`, runs the full suite as a regression
gate, and classifies failures as test bugs (self-fixed, up to 3
attempts) or code bugs (escalated back to the Django Developer). The
fix loop between Django Developer and Tester runs up to 3 cycles. On
...
correlate: a model that misremembers, say, which rounding rule a
tax calculation uses would write code with that bug *and* a test
expecting the same wrong number. Green checkmark, bug ships. Every
test's weak point is its **oracle** — wherever the expected value
came from — so the pipeline builds tests in three tiers, ordered by
how contamination-resistant that source is:

**Tier 1 — Properties and metamorphic relations (written first).**
Assertions with no computed expected value at all: object counts
preserved, results within documented bounds, invariance under input
reordering, spread unchanged by adding a constant, group means
between group min and max, exceptions raised where the spec says so,
a save/update being idempotent. Nothing was computed, so no
misconception can hide in the expectation. These catch implementation
slips (None handling, join duplication, N+1 evaluation order)
cheaply.
...
cases beyond the examples, the tester computes `expected` via plain
Python and explicit formulas — a deliberately different route from
the ORM/idiomatic implementation (see §1.10 of `STYLE_GUIDE.md` for
this carve-out). Both routes would need the same bug to agree
wrongly.
...
- Django Developer: reads test files but must never create or modify
  them
...
| `python-style` | Enforces Python/Django code conventions on all `.py` files |

> **Sync note:** the Python style rules now live in three places:
> `.project/STYLE_GUIDE.md` (canonical, for humans), the `python-style`
> skill (for ad-hoc Python work outside the pipeline), and inlined in
> `django-developer.md` and `tester.md` (so the pipeline agents pay no
...
3. Approve the plan. The Django Developer implements it.
...
2. Mirror the changes in
   `~/.config/opencode/skills/python-style/SKILL.md` (runtime copy),
   and in the inlined rules in `django-developer.md` and `tester.md`.

### Adding a Python Package

Add it to the Prerequisites section and pin it in `pyproject.toml`
or `requirements.txt`. Agents use explicit, absolute imports
throughout — no wildcard imports in any code produced by this setup.
