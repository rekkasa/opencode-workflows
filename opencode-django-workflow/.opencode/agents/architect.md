...
- `snake_case` for every function, variable, module, and Django field;
  `PascalCase` for classes, models, and serializers.
- Full type hints on every function signature (`str | None`, `list[int]`).
- Idiomatic Python/Django over hand-rolled: comprehensions not manual
  accumulation loops, `pathlib` not `os.path`, f-strings not `%`/`.format`,
  ORM querysets not raw SQL, `.first()`/`.get()` not `qs[0]`,
  `select_related`/`prefetch_related` to avoid N+1.
- Business logic in model methods or `services.py`, not views. Never
  hand-write schema migrations.
- 88-character line limit (black) — factor signatures accordingly.
...
[A numbered checklist divided into three phases:
 ...
 Phase 1 — Implementation: atomic coding and docstring steps, plus any
   `manage.py makemigrations` step (Django Developer).
 ...
 Format each pytest item as:
   N. [ ] [TEST] test_<module>.py: test <function>() <description>
   N. [ ] [TEST] test_<module>.py: run pytest tests/test_<module>.py and verify all pass]
```
...
```
Worked Examples:
  1. f(values=[2, 4, 6], drop_none=True)   ->  4
  2. f(values=[5, None], drop_none=False)  ->  None
```
...
- Inputs must be TINY (3–6 values) so the expected output can be derived
  ...
  exception type and message for an Errors example.
- Prefer inputs that exercise the interesting branch (a `None`, a tie, an
  empty queryset), not just the happy path.
